...
@mod_users.route('/users/<int:user_id>.json', methods=['GET'])
def user_by_id(user_id): 
    cur = mysql.connection.cursor()
    cur.execute('''SELECT id, email, first_name, last_name, user_type_id FROM users WHERE username = {}'''.format(user_id))
    row = cur.fetchone()
    if not row:
